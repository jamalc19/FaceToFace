...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.BatchNorm2d(32, momentum=0.1),
            nn.ReLU(),
            nn.Conv2d(in_channels=32, out_channels=1, kernel_size=7, stride=1, padding=3),
            # switch to 1 channel for black and white
            nn.Tanh()
        )
...
def train(generator, discriminator, num_epochs, checkpointfolder='', batchsize=32, lr=0.001,
    ...
    for epoch in range(start_epoch + 1, start_epoch + num_epochs + 1):
        ...
        for emotion_pics, labels in emotion_loader:
            ...
            labels = torch.tensor([classes_map[dataloaderclasses[i]] for i in
                                   labels])  # convert labels from dataloader indices to model indices
            ...
            feat_loss = FeatureCriterion(generated_features,
                                         neutral_features) * identity_loss_weight  # both input images are normalized
            ...
            generated_out = discriminator(
                generated_pics.clone().detach())  # discriminator takes normalized images -1,1.
            real_out = discriminator(emotion_pics)  # emotion loader already normalizes pics

            generatedD_loss = 0.5 * DiscriminatorCriterion(generated_out,
                                                           torch.zeros_like(generated_out)) * gan_loss_weight
...
if __name__ == "__main__":
    generator = Generator()
    discriminator = Discriminator()
    Losses = train(generator, discriminator, checkpointfolder='/Michael', num_epochs=4, batchsize=2, lr=0.001,
                   gan_loss_weight=30,
